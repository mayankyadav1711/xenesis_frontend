  ...
  />
      ...
      <div class="flex items-center justify-center max-w-[430px] w-[90%] h-full">
				<form action="/otpValidation/" method="post" class="form w-full h-full md:px-16 sm:px-12 px-8 lg:py-12 sm:py-11 py-10 backdrop-blur-md bg-slate-800 bg-opacity-15 shadow-inner border-solid border-x-4  border-neon-100 z-10 rounded-xl">
					{/* {%csrf_token%} */}
					<h1 class="text-white form-header-text text-center md:text-3xl sm:text-3xl text-xl font-bold uppercase text-transparent bg-clip-text bg-gradient-to-tr from-orange-500 to-yellow-300 mb-5 flex justify-center items-center  font-jost gap-4"><img src={ldrp} alt="" class="w-[50px] h-[50px]" />Enter OTP</h1>
                  ...
                  />
						...
						<div class="w-full flex items-center justify-center mb-3 text-white text-sm">
							<div class="flex items-center justify-start gap-2 w-fit">
								<p class="otp-text  font-jost">Didn't recieved the otp?</p>
								<button type="button" name="resendotp" class="text-red-400 font-jost text-lg font-semibold !outline-none resend-text" onclick="disableResendButton()">Resend Otp</button>
							</div>
						...
						<input type="submit" value="Verify" name="varification" tabindex="4" className='w-fit font-jost bg-neon-100 text-deepBlue-100 hover:shadow-2xl 3d-button hover:-translate-y-2 hover:border-neon-100  hover:saturate-200 outline-none mt-5 xl:px-16 lg:px-12 sm:px-10 px-9 py-1.5 text-[#f0e2ea] xl:text-lg lg:text-md font-semibold cursor-pointer transition-all duration-200 focus:-translate-y-1 hover:ring hover:ring-yellow-100 rounded-md'/>
					</div>
				</form>
			</div>

        {/* Link to Home Div */}
        <Link to="/" rel="noopener noreferrer" class="absolute top-5 left-5 flex flex-col items-center cursor-pointer hover:scale-[1.15] z-30 transition-all duration-300">
